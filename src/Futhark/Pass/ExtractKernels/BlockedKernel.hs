...
chunkedReduceKernel cs w step_one_size comm reduce_lam' fold_lam' nes arrs = do
  ...
  chunk_res_pes <- forM (lambdaReturnType fold_lam') $ \t -> do
    pe_name <- newVName "chunk_results"
    return $ PatElem pe_name BindVar $ t `arrayOfRow` group_size
  let fold_chunk = Thread chunk_res_pes $ lambdaBody fold_lam'
      (chunk_red_pes, chunk_map_pes) = splitAt (length nes) chunk_res_pes
  ...
  map_rets <- forM chunk_map_pes $ \pe ->
    return $ ConcatReturns w (kernelElementsPerThread step_one_size) $ patElemName pe
  let rets = red_rets ++ map_rets

  let red_ts = take (length nes) $ lambdaReturnType fold_lam'
  ...
  return $ Kernel cs
    ...
    ts thread_id $
    KernelBody (chunk_stm:[fold_chunk,reduce_chunk]) rets
...
reduceKernel cs step_two_size reduce_lam' nes arrs = do
  ...
  (copy_input, arrs') <-
    fmap unzip $ forM (zip (lambdaReturnType reduce_lam') arrs) $ \(t, arr) -> do
      arr' <- newVName (textual arr ++ "_copy")
      arr_index <- newVName (textual arr ++ "_index")
      return (Thread [PatElem arr' BindVar $ t `arrayOfRow` group_size] $
              Body () [Let (Pattern [] [PatElem arr_index BindVar t]) () $
  ...
  return $ Kernel cs
    ...
    KernelBody (copy_input++[reduce]) rets
